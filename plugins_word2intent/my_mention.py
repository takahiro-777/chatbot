...
def default_func(message):
    ...
    while node:
        ...
        if word in word2intent:
            tags.append(word2intent[word])
        #print('{0} , {1}'.format(word, pos))
        #次の単語に進める
        node = node.next
    if len(tags) > 0:
        message.reply("```Sentence you input is "+text+". Sentence intention is "+','.join(tags)+"```")      # メンション
        #message.send("Sentence tag is"+','.join(tags)+"```")
    else:
        message.reply("```すみません、わからないのでググってください```")
